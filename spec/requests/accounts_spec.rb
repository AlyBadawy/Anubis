...
RSpec.describe "accounts/", type: :request do
  ...
  describe "GET 'account/:username'" do
    context "when username exists" do
      it "renders a successful response" do
        ...
        get account_by_username_url("testUser"), as: :json, headers: @valid_headers
    ...
    context "when username does not exist" do
      ...
      it "renders a not found response" do
        get account_by_username_url(username), as: :json, headers: @valid_headers
  ...
  describe "PUT '/update'" do
    context "with a valid current password" do
      context "with valid parameters" do
        it "updates the requested user" do
          put update_profile_url,
              params: { user: { current_password: "password", first_name: "Updated", last_name: "One" } }, headers: @valid_headers, as: :json
        ...
        it "renders a JSON response with the updated user" do
          put update_profile_url,
              params: { user: { current_password: "password", first_name: "Updated", last_name: "Two" } }, headers: @valid_headers, as: :json
          expect(response).to have_http_status(:ok)
          expect(response.content_type).to match(a_string_including("application/json"))
        end
      ...
      context "with invalid parameters" do
        it "renders a JSON response with errors for the user" do
          put update_profile_url,
              params: { user: { current_password: "password", first_name: nil, last_name: nil } }, headers: @valid_headers, as: :json
          expect(response).to have_http_status(:unprocessable_entity)
          expect(response.content_type).to match(a_string_including("application/json"))
          res_body = JSON.parse(response.body)
    ...
    context "with invalid current password" do
      it "renders a JSON response with an error message" do
        put update_profile_url,
            params: { user: { current_password: "wrong_password", first_name: "Updated" } }, headers: @valid_headers, as: :json
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.content_type).to match(a_string_including("application/json"))
        res_body = JSON.parse(response.body)
  ...
  describe "Delete '/delete_account'" do
      ...
      it "deletes the user" do
        expect {
          delete delete_account_url,
                 params: { user: { current_password: "password" } }, headers: @valid_headers, as: :json
        }.to change(User, :count).by(-1)
      end
    ...
    context "with invalid current password" do
      it "renders a JSON response with an error message" do
        delete delete_account_url,
               params: { user: { current_password: "wrong_password" } }, headers: @valid_headers, as: :json
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.content_type).to match(a_string_including("application/json"))
        res_body = JSON.parse(response.body)
        expect(res_body["error"]).to eq("Current password is incorrect")
